...
class ControllerFeedLatestNewsRSS extends Controller {

	public function index() {
		if ($this->config->get('latest_news_rss_status')) {

			$this->load->model('catalog/module/news');
			$this->load->model('localisation/currency');
			$this->load->model('tool/image');
			$this->load->language('feed/latest_news_rss');

			$limit = $this->config->get('latest_news_rss_limit') ? $this->config->get('latest_news_rss_limit') : 100;
			$show_price = $this->config->get('latest_news_rss_show_price');
			$include_tax = $this->config->get('latest_news_rss_include_tax');
			$show_image = $this->config->get('latest_news_rss_show_image');


			if ($show_image) {
				$image_width = $this->config->get('latest_news_rss_image_width') ? $this->config->get('latest_news_rss_image_width') : 100;
				$image_height = $this->config->get('latest_news_rss_image_height') ? $this->config->get('latest_news_rss_image_height') : 100;
			}
          ...
          $output .= '<pubDate>' . date(DateTime::RSS, strtotime($product['date_added'])) . '</pubDate>';
					$output .= '</item>';
				}
			}
