...
class ControllerModuleViewed extends Controller {
	protected function index() {
		...
		$results = $this->model_catalog_viewed->getMostViewedProducts($this->config->get('viewed_limit'));
		...
		if ($this->config->get('viewed_position') == 'home') {
			$this->data['heading_title'] .= (' ' . $this->language->get('text_products'));
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/viewed_home.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/module/viewed_home.tpl';
			} else {
				$this->template = 'default/template/module/viewed_home.tpl';
			}
		} else {
