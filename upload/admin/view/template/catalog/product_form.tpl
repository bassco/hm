...
<div id="content">
  ...
  <div class="box">
    <div class="heading">
      <h1><img src="view/image/product.png" alt="" /> <?php echo $heading_title . $heading_title_name; ?></h1>
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <div id="tab-links">
          <table class="form">
            ...
            <tr>
              <td><?php echo $entry_spares; ?></td>
              <td><input type="text" name="spares" value="" /></td>
            </tr>
            <tr>
              <td>&nbsp;</td>
              <td><div id="product-spares" class="scrollbox">
                  <?php $class = 'odd'; ?>
                  <?php foreach ($product_spares as $product_spares) { ?>
                  <?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
                  <div id="product-spares<?php echo $product_spares['product_id']; ?>" class="<?php echo $class; ?>"> <?php echo $product_spares['name']; ?><img src="view/image/delete.png" />
                    <input type="hidden" name="product_spares[]" value="<?php echo $product_spares['product_id']; ?>" />
                  </div>
                  <?php } ?>
                </div></td>
            </tr>
...
$('input[name=\'spares\']').autocomplete({
	delay: 0,
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/product/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term),
	...
	select: function(event, ui) {
		$('#product-spares' + ui.item.value).remove();
		
		$('#product-spares').append('<div id="product-spares' + ui.item.value + '">' + ui.item.label + '<img src="view/image/delete.png" /><input type="hidden" name="product_spares[]" value="' + ui.item.value + '" /></div>');

		$('#product-spares div:odd').attr('class', 'odd');
		$('#product-spares div:even').attr('class', 'even');
...
$('#product-spares div img').live('click', function() {
	$(this).parent().remove();
