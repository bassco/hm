...
class ModelReportCustomer extends Model {
    ...
    public function getCredit($data = array()) {
        ...
        $sql .= " GROUP BY ct.customer_id HAVING SUM(ct.amount) <> 0 ORDER BY total DESC";
    ...
    public function getTotalCredit($data) {
        $sql = "SELECT SUM(amount) AS total FROM `" . DB_PREFIX . "customer_transaction`";

        $subsql = " customer_id IN (SELECT customer_id FROM `" .DB_PREFIX ."customer_transaction` ";
        ...
        if ($implode) {
            $subsql .= " WHERE " . implode(" AND ", $implode);
        }

        $subsql .= " GROUP BY customer_id HAVING SUM(amount) <> 0 ) ";
    ...
    public function getCountCredit($data) {
    ...
    public function getOrderSummary($data){
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "vw_order_summary`");
        return $query->rows;
    }
