...
class ModelSaleCustomer extends Model {
    ...
    public function getTotalCustomersByCurrentMonth() {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE (status = '1' OR approved = '1') and DATE(date_added) >= concat(Year(Current_date),'-',Month(Current_Date),'-01')");
    ...
    public function getProductsByCustomerId($customer_id) {
		...
		$sql .= " WHERE o.customer_id = '" . (int)$customer_id . "' AND o.order_status_id > '0'";
		
		$sql .= " GROUP BY op.name, op.model ORDER BY op.name ASC";
