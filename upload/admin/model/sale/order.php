			...
			foreach($product_query->rows as $product) {
				$this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity = (quantity + " . (int)$product['quantity'] . ") WHERE product_id = '" . (int)$product['product_id'] . "' AND subtract = '1'");
                                ...
                                if ($this->db->countAffected() > 0) {
                                        $result = $this->db->query("SELECT quantity FROM " . DB_PREFIX . "product WHERE product_id = " . (int)$product['product_id']);
                                        $newdata = $result->row;
...
date) VALUES (
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.payment_code, o.payment_method, o.shipping_method, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
		...
		if (!empty($data['filter_date_added'])) {
			...
			$sql .= " AND LCASE(o.payment_code) ='" . $this->db->escape(utf8_strtolower($data['filter_payment_code'])) . "'";
		}
	...
	public function getTotalOrders($data = array()) {
      	$sql = "SELECT COUNT(1) AS total FROM `" . DB_PREFIX . "order`";
		...
		if (!empty($data['filter_payment_code'])) {
			$sql .= " AND LCASE(payment_code) = '" . $this->db->escape($data['filter_payment_code']) . "'";
		}
	...
	public function getTotalOrdersByStoreId($store_id) {
      	$query = $this->db->query("SELECT COUNT(1) AS total FROM `" . DB_PREFIX . "order` WHERE store_id = '" . (int)$store_id . "'");

		return $query->row['total'];
	}

	public function getTotalOrdersByOrderStatusId($order_status_id) {
      	$query = $this->db->query("SELECT COUNT(1) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id = '" . (int)$order_status_id . "' AND order_status_id > '0'");

		return $query->row['total'];
	}

	public function getTotalOrdersByLanguageId($language_id) {
      	$query = $this->db->query("SELECT COUNT(1) AS total FROM `" . DB_PREFIX . "order` WHERE language_id = '" . (int)$language_id . "' AND order_status_id > '0'");

		return $query->row['total'];
	}

	public function getTotalOrdersByCurrencyId($currency_id) {
      	$query = $this->db->query("SELECT COUNT(1) AS total FROM `" . DB_PREFIX . "order` WHERE currency_id = '" . (int)$currency_id . "' AND order_status_id > '0'");

		return $query->row['total'];
	}

	public function getTotalStock() {
      	$query = $this->db->query("SELECT SUM(quantity * price) AS total FROM `" . DB_PREFIX . "product` WHERE status > '0'");
    ...
    public function getTotalOrdersByCurrentMonth() {
        $query = $this->db->query("SELECT COUNT(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND DATE(date_added) >= concat(Year(Current_date),'-',Month(Current_Date),'-01')");
		return $query->row['total'];
	}

    public function getTotalSalesByCurrentMonth() {
        $query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND DATE(date_added) >= concat(Year(Current_date),'-',Month(Current_Date),'-01')");
		return $query->row['total'];
	}

	public function getTotalSalesByFinYear() {
      	$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND case when MONTH(CURRENT_DATE) >= 3 then DATE(date_added) BETWEEN  concat(Year(Current_date),'-03-01') and concat(Year(Current_date),'-03-01') + interval 1 year  - interval 1 day ELSE DATE(order.date_added) BETWEEN concat(Year(Current_date),'-03-01') - interval 1 year  - interval 1 day AND concat(Year(Current_date),'-03-01')  end");
		return $query->row['total'];
	}

    public function getTotalOrdersByFinYear() {
      	//$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND YEAR(date_added) = '" . (int)$year . "'");
        $query = $this->db->query("SELECT COUNT(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND case when MONTH(CURRENT_DATE) >= 3 then DATE(date_added) BETWEEN  concat(Year(Current_date),'-03-01') and concat(Year(Current_date),'-03-01') + interval 1 year  - interval 1 day ELSE DATE(order.date_added) BETWEEN concat(Year(Current_date),'-03-01') - interval 1 year  - interval 1 day AND concat(Year(Current_date),'-03-01')  end");
		return $query->row['total'];
	}

	public function getTotalMonthlySales(){
		$query = $this->db->query("SELECT COUNT(1) as count, SUM(total) AS total, YEAR(order.date_added) as year , MONTH(date_added) as month FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' group by YEAR(date_added), MONTH(date_added) order by YEAR(date_added), MONTH(date_added)");
		return $query->rows;
	}
	...
	public function getTotalOrderHistories($order_id) {
	  	$query = $this->db->query("SELECT COUNT(1) AS total FROM " . DB_PREFIX . "order_history WHERE order_id = '" . (int)$order_id . "'");

		return $query->row['total'];
	}

	public function getTotalOrderHistoriesByOrderStatusId($order_status_id) {
	  	$query = $this->db->query("SELECT COUNT(1) AS total FROM " . DB_PREFIX . "order_history WHERE order_status_id = '" . (int)$order_status_id . "'");
    ...
    public function getOrderPaymentMethods() {
		$query = $this->db->query("SELECT DISTINCT payment_code, payment_method FROM `" . DB_PREFIX . "order`");
