...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_spares'])) {
			foreach ($data['product_spares'] as $spares_id) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_spares WHERE product_id = '" . (int)$product_id . "' AND spares_id = '" . (int)$spares_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_spares SET product_id = '" . (int)$product_id . "', spares_id = '" . (int)$spares_id . "'");
			}
		}
	...
	public function editProduct($product_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_spares WHERE product_id = '" . (int)$product_id . "'");
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			$data = array_merge($data, array('product_spares' => $this->getProductSpares($product_id)));
	...
	public function getProducts($data = array()) {
		if ($data) {
			...
			if (!empty($data['filter_name'])) {
				...
				$terms = explode(' ', $data['filter_name']);

				$bits = array();
				foreach ($terms as $term) {
				    $bits[] = "LCASE(pd.name) LIKE '%".$this->db->escape(utf8_strtolower($term))."%'";
				}

	//			$sql .= " AND LCASE(pd.name) LIKE '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "%'";
				$sql .= " AND  (".implode(' AND ', $bits).") ";
			}
	...
	public function getProductSpares($product_id) {
		$product_spares_data = array();
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_spares WHERE product_id = '" . (int)$product_id . "'");
		
		foreach ($query->rows as $result) {
			$product_spares_data[] = $result['spares_id'];
		}
		
		return $product_spares_data;
	}
	...
	public function getPurchasedTotalByProductId($product_id){
		$query = $this->db->query("SELECT SUM(op.total + op.total * op.tax / 100) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) WHERE o.order_status_id > '0' AND op.product_id='" . (int)$product_id . "'");
 
   		return $query->row['total'];
	}
    
	public function getPurchasedCountByProductId($product_id){
