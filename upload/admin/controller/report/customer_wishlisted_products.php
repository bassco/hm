<?php
class ControllerReportCustomerWishlistedProducts extends Controller { 
	public function index() {  
		...
		if ($back = 1) {
			...
			$this->data['breadcrumbs'][] = array(
	   		...
	   		$this->data['breadcrumbs'][] = array(
				...
				'href'      => $this->url->link('report/customer_wishlists', 'token=' . $this->session->data['token'], 'SSL'),
	      		'separator' => ' :: '
	   		);		
	
	   		$this->data['breadcrumbs'][] = array(
	       		'text'      => $this->language->get('heading_title_products'),
				'href'      => $this->url->link('report/customer_wishlisted_products', 'token=' . $this->session->data['token'] . '&customer_id=' . $customer_id . '&back=1', 'SSL'),
	      		'separator' => ' :: '
	   		);
   			...
   			$this->data['breadcrumbs'][] = array(
	       		'text'      => $this->language->get('heading_title_products'),
				'href'      => $this->url->link('report/customer_wishlisted_products', 'token=' . $this->session->data['token'], 'SSL'),
	      		'separator' => ' :: '
	   		);		
		...
		$wlproducts = $this->model_report_wishlisted->getCustWishlist($customer_id);
		
		$custname = $this->model_report_wishlisted->getwlCustomer($customer_id);
		...
		$wishlisted_total = $this->model_report_wishlisted->getWishlistedTotal($customer_id);

		foreach ($wlproducts as $wlproduct) {
		
			$results = $this->model_report_wishlisted->getProduct($wlproduct);
		...
		$pagination->total = $wishlisted_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('report/customer_wishlisted_products', 'token=' . $this->session->data['token'] . '&customer_id=' . $customer_id . '&page={page}');
