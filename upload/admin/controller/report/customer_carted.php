<?php
class ControllerReportCustomerCarted extends Controller { 
	public function index() {  
		...
		$wlcustomers = $this->model_report_carts->getCustomerCarted($item);
		
		$customer_total = $this->model_report_carts->getCustomerTotal($item);

		foreach ($wlcustomers as $wlcustomer) {
			$results = $this->model_report_carts->getwlCustomer($wlcustomer);
			  ...
			  $view[] = array(
				...
				'href' => $this->url->link('report/customer_carted_products', 'token=' . $this->session->data['token'] . '&customer_id=' . $results['customer_id'] . '&wlitem=' . $item, 'SSL'),
			  );
		...
		$pagination->url = $this->url->link('report/customer_carted', 'token=' . $this->session->data['token'] . '&wlitem=' . $item . '&page={page}');
