...
class ControllerSaleOrder extends Controller {
	private $error = array();
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_payment_code'])) {
				$url .= '&filter_payment_code=' . urlencode(html_entity_decode($this->request->get['filter_payment_code'], ENT_QUOTES, 'UTF-8'));
			}
  	...
  	private function getList() {
		...
		$this->data['shipped'] = $this->url->link('sale/order/setShippedOrderStatus', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['delivery_box'] = $this->url->link('sale/order/delivery_box', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['delivery'] = $this->url->link('sale/order/delivery', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['button_shipped'] = $this->language->get('button_shipped');
        $this->data['button_delivery_box'] = $this->language->get('button_delivery_box');
		...
		$this->data['sort_payment_code'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.payment_code' . $url, 'SSL');
		...
		$this->data['filter_payment_code'] = $filter_payment_code;
        ...
        $this->data['order_payment_methods'] = $this->model_sale_order->getOrderPaymentMethods();
	...
	public function info() {
		...
		if ($order_info) {
			...
			$this->data['delivery_box'] = $this->url->link('sale/order/delivery_box', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$this->data['delivery'] = $this->url->link('sale/order/delivery', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
		    ...
		    if (isset($this->request->post['firstname'])) {
                $this->data['heading_title'] .= " - " . $data['firstname'];
		    }
            if (isset($this->request->post['lastname'])) {
                $this->data['heading_title'] .= " " . $data['lastname'];
		    }
			...
			if ($order_info['customer_id']) {
				...
				$this->data['customer_id'] = $order_info['customer_id'];
				$this->data['login'] = $this->url->link('account/login', 'token=' . $this->session->data['token'] . '&customer_id=' . $order_info['customer_id'], 'SSL');
			} else {
				$this->data['customer'] = '';
				$this->data['customer_id'] = '';
				$this->data['login'] = '';
    ...
    public function delivery() {
		...
		$this->template = 'sale/order_delivery.tpl';
  	...
  	public function delivery_box() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
                ...
                $text_delivery = $this->language->get('text_delivery') . ' - ' . $order_id;
		...
		$this->template = 'sale/order_delivery_box.tpl';

		$this->response->setOutput($this->render());
	}

    function setShippedOrderStatus(){
        $this->load->model('sale/order');
        ...
        $data=['notify'=>1, 'order_status_id'=>'3', 'comment'=>'' ];
        foreach ($orders as $order_id)
        {
            # Set the shipped status for the orders
