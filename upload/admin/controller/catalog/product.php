...
class ControllerCatalogProduct extends Controller {
	private $error = array(); 
  	...
  	private function getList() {				
		...
		foreach ($results as $result) {
			...
			foreach ($product_specials  as $product_special) {
      		...
      		$this->data['products'][] = array(
                ...
                'sales'      => $this->model_catalog_product->getPurchasedCountByProductId($result['product_id']),
				'price'      => $result['price'],
				'special'    => $special,
				'image'      => $image,
		...
		$this->data['sort_viewed'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.viewed' . $url, 'SSL');
  	...
  	private function getForm() {
		...
		$this->data['entry_spares'] = $this->language->get('entry_spares');
        ...
        if(count($this->data['product_description'])){
            $this->data['heading_title_name'] = " - ". $this->data['product_description'][$this->config->get('config_language_id')]['name'];
        }else{
            $this->data['heading_title_name'] = "";
        }
		...
		if (isset($this->request->post['product_spares'])) {
			$products = $this->request->post['product_spares'];
		} elseif (isset($this->request->get['product_id'])) {		
			$products = $this->model_catalog_product->getProductSpares($this->request->get['product_id']);
		} else {
			$products = array();
		}
	
		$this->data['product_spares'] = array();
		
		foreach ($products as $product_id) {
			$spares_info = $this->model_catalog_product->getProduct($product_id);
			
			if ($spares_info) {
				$this->data['product_spares'][] = array(
					'product_id' => $spares_info['product_id'],
					'name'       => $spares_info['name']
				);
			}
		}
    ...
    public function ajax_product_orders() {
		$json = array();
