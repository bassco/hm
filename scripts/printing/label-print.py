...
"""Read an order from the DB and print a shipping label
...
#  t=Template(data)
  ...
  with open("/dev/usb/%s" % fn,"wb") as fh:
    fh.write(fr.read())
    for lab in labels:
      try:
        fh.write(lab.label())
...
class Label():
    ...
    def name(self):
        ...
        s[-1] = s[-1].capitalize()
	s = " ".join([self._firstname.strip().capitalize(), " ".join(s)])
	t = s.split("'")
	if len(t) > 1:
		s = "'".join([ n.capitalize() for n in t ])
        return s

    @property
    def company(self):
        s = self.unescape(self._company).strip().upper()
        if len(s) > 30: raise AttributeError("Order %s: Company too long." % self.order_id)
        return s

    @property
    def address_1(self):
        s =  self.unescape(self._address_1).strip().title()
        if len(s) > 33: raise AttributeError("Order %s: Address 1 too long (%d)." % (self.order_id, len(s)))
        return s

    @property
    def address_2(self):
        s =  self.unescape(self._address_2).strip().title()
        if len(s) > 33: raise AttributeError("Order %s: Address 2 too long." % self.order_id)
        return s

    @property
    def city(self):
        s =  self.unescape(self._city).strip().title()
        if len(s) > 30: raise AttributeError("Order %s: City too long." % self.order_id)
        return s

    @property
    def zone(self):
        return self.unescape(self._zone).strip().upper()
